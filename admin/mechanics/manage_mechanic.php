...
if(isset($_GET['id']) && $_GET['id'] > 0){
    ...
    if($qry->num_rows > 0){
        ...
        echo "<script>console.log('Product Name: " . addslashes($name) . "');</script>";
    }
}
?>
    ...
    <div class="card-body">
        <form action="" id="mechanic-form">
            ...
            <div class="form-group">
                ...
                <input name="nic" id="nic" type="text" class="form-control rounded-0" value="<?php echo isset($nic) ? $nic : ''; ?>" required>
            </div>
            <div class="form-group">
    ...
    $(document).ready(function(){
        $('#mechanic-form').submit(function(e){
            ...
            if(!/^\d{10}$/.test(contact)){
                isValid = false;
                errorMsg += "Contact must be exactly 10 digits.<br>";
            }

            // Validate NIC
            if(!/^[a-zA-Z0-9]{10,11}$/.test(nic)){
                isValid = false;
                errorMsg += "NIC must be between 10 to 11 characters and contain only letters and digits.<br>";
            }

            if(!isValid){
                var el = $('<div>');
                el.addClass("alert alert-danger err-msg").html(errorMsg);
                _this.prepend(el);
                el.show('slow');
